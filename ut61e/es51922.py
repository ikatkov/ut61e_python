...
FUNCTION = {
    ...
    # Auto μA Current / Auto μA Current / Auto 220.00A/2200.0A
    0b0111101: ("current", RANGE_CURRENT_AUTO_UA, "A"),
    # Auto mA Current   Auto mA Current   Auto 22.000A/220.00A
    0b0111111: ("current", RANGE_CURRENT_AUTO_MA, "A"),
...
def parse(packet, extended_format=False):
    ...
    d_range, \
        ...
        d_function, d_status, \
        d_option1, d_option2, d_option3, d_option4 = struct.unpack(
            "B"*12, packet)
...
def output_readable(results):
    ...
    if operation == "normal":
        ...
        line = "-, the measurement is {operation}ed!".format(
            operation=operation)
    if battery_low:
...
def output_csv(results):
    ...
    """
    field_data = [format_field(results, field_name)
...
def read_raw_data(ser):
    ...
    for x in range(READ_RETRIES):
        raw_data = ser.read_until(EOL, RAW_DATA_LENGTH)
    ...
    #     res = raw_data.decode('ascii')
    #     # res = [ord(c) for c in bytes(raw_data).decode(encoding = 'ascii')]

    return raw_data[:-2].decode('ascii')


def is_data_valid(raw_data):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Utility for parsing data from multimeters based on Cyrustek ES51922 chipset.')
    ...
    parser.add_argument('--oneshot', action='store_true', default=False,
                        help='Read one value from serial port and exit. Specify port as --port %port%. Default is /dev/cu.usbserial-1440.')
    parser.add_argument('-p', '--port', default='/dev/cu.usbserial-1440',
                        help='Serial port for the multimeter. Default is /dev/cu.usbserial-1440.')
    ...
    if (not args.oneshot):
        logging.info('Reading values in a loop')
        ...
        while True:
            ...
            try:
                ...
                logging.warning(
                    'Not an ASCII input line, ignoring: "{}"'.format(line))
                continue
            timestamp = datetime.datetime.now()
            ...
            if len(line) == 12:
                try:
                    ...
                    logging.warning(
                        'Error "{}" packet from multimeter: "{}"'.format(e, line))
                if args.mode == 'csv':
                ...
                logging.warning(
                    'Unknown packet from multimeter: "{}"'.format(line))
            else:
                logging.warning(
                    'Not a response from the multimeter: ""'.format(line))
    else:
        logging.info('Reading one value from serial port and exiting.')
        ser = serial.Serial(args.port, BAUD_RATE, BITS, PARITY,
                            STOP_BITS, timeout=TIMEOUT)
        line = read_raw_data(ser)
        try:
            line = line.encode('ascii')
        except:
            logging.warning(
